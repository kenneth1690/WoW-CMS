		...
		<div id="wm-error-page">
			<?php 
				session_start();
					...
					$cmssql= "SELECT * FROM news WHERE id='$chgsidcheck'";
					$resultcms = mysqli_query($cmsconn,$cmssql);
					$rowscms = mysqli_fetch_array($resultcms);
					...
					if($rowscms['id']==$chgsidcheck){
							$sqldate3 = "DELETE FROM `changelogs` WHERE `id`=".$_GET['chgsid'];
							...
							$insertlog = mysqli_query($cmsconn, "INSERT INTO logs_gm (`logger`, `logger_id`, `logger_gmlevel`, `logdetails`, `logdate`) 
									  VALUES ('".$_SESSION['loggedin']."', '".$rows['id']."', '".$rowsgm['gmlevel']."', 'CHANGELOGS: User `".$nick."` deleted changelogs (CHID: ".$_GET['chgsid'].")', NOW());");
							header("refresh:5;url=index.php");
							?>
							<center>
							<p><font size="6">Changelogs deleted</font></p>
							<p>
								<font size="5">Your changelogs has been deleted from the database.</font>
							</p> 
							</center>
							<?php
					}else{
						...
						<p>
							<font size="5">You tried to delete changelogs that not exists.</font>
						</p> 
						</center>
						<?php
					}
