			...
			<?php 
				session_start();
					...
					if($rowscms['id']==$newsidcheck){
						if(empty($edittitlecheck) && empty($editcontentcheck)){
							header("refresh:5;url=index.php");
							?>
							<center>
							<p><font size="6">Error when editing</font></p>
							<p>
								<font size="5">You must edit at least 'title' or 'content' field.</font>
							</p> 
							</center>
							<?php
						}else{
							if(empty($edittitlecheck)){
								$sqldate1 = "UPDATE `news` SET `content`='$editcontentcheck',`edited_by`='$nick',`edited_date`=NOW() WHERE `id`=".$_GET['newsid'];
								mysqli_query($cmsconn, $sqldate1);
							}elseif(empty($editcontentcheck)){
								$sqldate2 = "UPDATE `news` SET `title`='$edittitlecheck',`edited_by`='$nick',`edited_date`=NOW() WHERE `id`=".$_GET['newsid'];
								mysqli_query($cmsconn, $sqldate2);
							}else{
							...
							?>
							<center>
							<p><font size="6">News edited</font></p>
							<p>
								<font size="5">Your news has been edited in the database.</font>
							</p> 
							</center>
							<?php
						}
