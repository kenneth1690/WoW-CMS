...
if(!isset($_SESSION["loggedin"]) || empty($_SESSION["loggedin"])){
	...
	$title = mysqli_real_escape_string($con, $row3['title']);
				...
				if($rowsgm['gmlevel']>0){
					if($tid){
						if($row3['pinned']==0){
							$insert2 = mysqli_query($con, "UPDATE `topics` SET `pinned`='1' WHERE `topic_id`='$tid'");
							$insertlog = mysqli_query($con, "INSERT INTO logs_gm (`logger`, `logger_id`, `logger_gmlevel`, `logdetails`, `logdate`) 
									  VALUES ('".$_SESSION['loggedin']."', '".$rows['id']."', '".$rowsgm['gmlevel']."', 'TOPIC: Pinned Topic: `$title` (CID: ".$cid." / SCID: ".$scid." / TID: ".$tid.")', NOW());");
						}elseif($row3['pinned']==1){
							$insert2 = mysqli_query($con, "UPDATE `topics` SET `pinned`='0' WHERE `topic_id`='$tid'");
							$insertlog = mysqli_query($con, "INSERT INTO logs_gm (`logger`, `logger_id`, `logger_gmlevel`, `logdetails`, `logdate`) 
									  VALUES ('".$_SESSION['loggedin']."', '".$rows['id']."', '".$rowsgm['gmlevel']."', 'TOPIC: Unpinned Topic: `$title` (CID: ".$cid." / SCID: ".$scid." / TID: ".$tid.")', NOW());");
						}
					}else{
